...
export const useInjectionVolumeOverTimeChart = () => {
    ...
    const setHydrationPeriodData = useCurrentDevice((state) => state.setHydrationPeriodData);
    ...
    const options = useMemo(() => {
        return {
            ...
            xAxis: {
                ...
                plotBands: hydrationPeriodData.map((band) => ({ from: band.from, to: band.to, color: 'red' }))
            },
            yAxis: [
    ...
    }, [injectionVolumeOverTimeData, valveOpeningPercentageOverTimeData, hydrationPeriodData]);
    ...
    const fetchHistoricalData = useCallback(async () => {
        try {
            ...
            response.data.data.forEach((point) => {
                ...
                injectionVolumeOverTimeChunk.push([timeStamp, point.gas_meter_volume_instant]);
                valveOpeningPercentageOverTimeChunk.push([timeStamp, point.gas_valve_percent_open]);
            });
    ...
    }, [currentDeviceId, injectionVolumeOverTimeData, hydrationPeriodData, axiosInstance]);
